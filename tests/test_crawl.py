...
handlers; a Crawler runs the worker pool to completion (finish() drains the
queue). A MockTransport serves a shallow crawl (webfinger -> actor -> count-only
...
Assumed contract (flag if different):
  make_dispatcher(redis, graph, *, transport=None, private_key_pem_data) -> Dispatcher
      (counters, clients, all four handlers registered — the test passes a
       ...
       contents; the rest is crawl.py's to build)
  Crawler(dispatcher, max_workers); await start(); await finish()
      start() creates the worker pool; finish() drains the queue (join) and
      winds the workers down -- run to completion.
"""

import httpx
from fakeredis import FakeAsyncRedis, FakeServer

from crawl import make_dispatcher
...
async def test_crawl_builds_the_graph_from_a_seed(keypair):
    ...
    crawler = Crawler(dispatcher, max_workers=4)
    await crawler.start()
